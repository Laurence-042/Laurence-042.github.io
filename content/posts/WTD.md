...
你问我为啥不用MTool或RenpyThief？TMD那个神人作者TMD拿Light.VN做了个TMD超棒的TMD黄油，但资源做了加密，也根本没考虑国际化，然后这俩为了特定引擎特化的工具都搞不定，就算能翻译也是外挂文本框，把原文和翻译结果对一起都是个麻烦事。而WindowTranslator翻译没个上下文，翻译结果看上去是根据本地OCR结果翻译得到的，质量很糟糕。Google 有拍照翻译功能，但那个没有公开 API，Cloud Translation 只接受文本。所以更务实的方案是本地用 manga-ocr 做高质量日文识别，再把文本发给翻译服务——OCR 和翻译分开，各用最合适的工具
